...
def simple_pouring(listCups, simulation):
    listCups_center = []
    listCups_edge = []
    listCups_alpha = []
    listCups_moved = []
    ...
    for i, obj in enumerate(listCups, 0):
        ...
        listCups_endlocations.append([g.xLoc, g.yLoc+(i)*g.dCups])

    position_cups(listCups_moved, listCups_center, listCups_alpha, listCups_endlocations, simulation)
    pouring_cups(listCups_moved, listCups_center, listCups_alpha, listCups_endlocations, simulation)
...
def position_cups(listCups_moved, listCups_center,listCups_alpha, listCups_endlocations, simulation):
    ...
    while not (check(listCups_moved)):
        for i in range(len(listCups_center)):
            ...
            if nb_obst == 0:
                ...
                angle = 0
                move_robot.grab_cup(listCups_center[i][0], listCups_center[i][1], angle, simulation)
                move_robot.place_cup(listCups_endlocations[i][0], listCups_endlocations[i][1], simulation)
...
def pouring_cups(listCups_moved, listCups_center,listCups_alpha, listCups_endlocations, simulation):

    #Pour cup 1 into cup 2
    print("Pouring cups")
    x = 0
    angle = math.pi/2
    move_robot.grab_cup(listCups_center[x][0], listCups_center[x][1], angle, simulation)
    move_robot.place_cupP(0.2, 0.5, simulation)
    listCups_alpha[x] += angle
    #angle that wrist is supposed to take
    print(listCups_alpha[x])

    move_robot.grab_cupP(0.2, 0.5, listCups_alpha[x], simulation)
    move_robot.pouring(listCups_endlocations[1][0],listCups_endlocations[1][1], simulation)
    return
